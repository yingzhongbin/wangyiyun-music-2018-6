    ...
    let view = {
        ...
        render(data){
            ...
            for(let i = 0;i<lyricAndTime.length;i++){
                let minute = lyricAndTime[i].substr(1,2)
                let second = parseFloat(lyricAndTime[i].substr(4,5),10) + parseInt(minute,10)*60
                timeBreak.push(second)

                lyricLines.push(lyricAndTime[i].substr(10))
                $p = $("<p></p>")
                $p.text(lyricAndTime[i].substr(10))
